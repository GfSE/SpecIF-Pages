...
//						console.debug('#5b', ch );
	...
	/*	// c) Add Actors, States and Events to the respective folders in alphabetical order:
        ...
        // console.debug('NodeList', nodeL);

        // d) Sort the categories in the hierarchy:
        //    - Omit the first subfolder with the diagrams (views), thus start with i=1
        for(let i=1; i<nodeL[0].nodes.length; i++) {
            nodeL[0].nodes[i].nodes.sort(function(a, b) {
                let titleA = itemById(resL,a.resource).title.toLowerCase();
                let titleB = itemById(resL,b.resource).title.toLowerCase();
