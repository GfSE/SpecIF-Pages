...
class CProject {
    ...
    readItems(ctg, itemL, options) {
        const opts = {
            reload: options?.reload ?? false,
            timelag: options?.timelag ?? 10,
            extendClasses: options?.extendClasses ?? false,
            showEmptyProperties: options?.showEmptyProperties ?? false
        };
        let self = this;
        return new Promise((resolve) => {
            ...
            switch (ctg) {
                ...
                default:
                    if (this.cache.delete(ctg, itmL))
                        break;
                    reject(new resultMsg({ status: 999, statusText: 'One or more items of ' + ctg + ' not found and thus not deleted.' }));
                    return;
            }
            ;
            resolve();
    ...
    exportAs(opts) {
        ...
        return new Promise((resolve, reject) => {
            if (self.exporting) {
                reject(new resultMsg({ status: 999, statusText: "Export in process, please wait a little while." }));
            }
            else {
                self.exporting = true;
                switch (opts.format) {
            ...
            function storeAs(opts) {
                ...
                switch (opts.format) {
                    ...
                    case 'specifClasses':
                        break;
                    default:
                        throw new Error("Programming error: Invalid format selector on export.");
                }
                ;
                self.read(opts)
                    .then((expD) => {
                    ...
                    switch (opts.format) {
                        ...
                        case 'specifClasses':
                            ...
                            if (!Array.isArray(opts.domains) || opts.domains.length < 1) {
                                reject(new resultMsg({ status: 999, statusText: "No domain selected, so no classes will be generated." }));
                                return;
                            }
                            ;
    ...
    compatibleDT(refC, newC) {
            ...
            switch (newC.type) {
                ...
                case XsDataType.Double:
                    if (refC.fractionDigits < newC.fractionDigits) {
                        new resultMsg({ status: 952, statusText: "new dataType '" + newC.id + "' of type '" + newC.type + "' is incompatible" }).log();
                        return false;
                    }
                    ;
                case XsDataType.Integer:
                    if (refC.maxInclusive < newC.maxInclusive || refC.minInclusive > newC.minInclusive) {
                        new resultMsg({ status: 953, statusText: "new dataType '" + newC.id + "' of type '" + newC.type + "' is incompatible" }).log();
                        return false;
                    }
                    ;
                    break;
                case XsDataType.String:
                    if (refC.maxLength && (newC.maxLength == undefined || refC.maxLength < newC.maxLength)) {
                        new resultMsg({ status: 951, statusText: "new dataType '" + newC.id + "' of type '" + newC.type + "' is incompatible" }).log();
                        return false;
                        ;
                    }
                    ;
        ...
        function compatibleEnumeration(refC, newC) {
            ...
            for (var v = newC.enumeration.length - 1; v > -1; v--) {
                ...
                if (idx < 0) {
                    new resultMsg({ status: 954, statusText: "new dataType '" + newC.id + "' of type '" + newC.type + "' is incompatible" }).log();
                    return false;
                }
                ;
            }
    ...
    compatiblePC(refC, newC) {
        if (LIB.equalPC(refC, newC))
            return true;
        new resultMsg({ status: 956, statusText: "new propertyClass '" + newC.id + "' is incompatible" }).log();
        return false;
    }
    ...
    compatibleRC(refC, newC, opts) {
        if (this.compatiblePCReferences(refC.propertyClasses, newC.propertyClasses, opts))
            return true;
        new resultMsg({ status: 963, statusText: "new resourceClass '" + newC.id + "' is incompatible; propertyClasses don't match" }).log();
        return false;
    }
    compatibleSC(refC, newC, opts) {
        if (refC.title != newC.title) {
            new resultMsg({ status: 961, statusText: "new statementClass '" + newC.id + "' is incompatible; titles don't match" }).log();
            return false;
        }
        if (!this.compatibleECReferences(refC.subjectClasses, newC.subjectClasses)) {
            new resultMsg({ status: 962, statusText: "new statementClass '" + newC.id + "' is incompatible; subjectClasses don't match" }).log();
            return false;
        }
        ;
        if (!this.compatibleECReferences(refC.objectClasses, newC.objectClasses)) {
            new resultMsg({ status: 962, statusText: "new statementClass '" + newC.id + "' is incompatible; objectClasses don't match" }).log();
            return false;
        }
        ;
        if (this.compatiblePCReferences(refC.propertyClasses, newC.propertyClasses, opts))
            return true;
        new resultMsg({ status: 963, statusText: "new statementClass '" + newC.id + "' is incompatible; propertyClasses don't match" }).log();
        return false;
    }
