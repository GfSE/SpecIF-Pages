...
function sysml2specif(xmi, options) {
    ...
    function cameo2specif(xmi, opts) {
        ...
        stateTransitionL.forEach((stT) => {
            let tr = LIB.itemById(spD.resources, stT.id), trTi = LIB.valueByTitle(tr, DcProperty.title, spD), trTy = LIB.valueByTitle(tr, DcProperty.type, spD), src = LIB.itemById(spD.resources, stT.source), sTi = LIB.valueByTitle(src, DcProperty.title, spD), sTy = LIB.valueByTitle(src, DcProperty.type, spD), tgt = LIB.itemById(spD.resources, stT.target), tTi = LIB.valueByTitle(tgt, DcProperty.title, spD);
            if (sTy == 'uml:Pseudostate' && sTi.includes('unnamed')) {
                sTi = 'entry';
                LIB.updatePropertyByTitle(src, DcProperty.title, () => { return [[{ text: sTi }]]; }, spD);
            }
            ;
            if (trTy == 'uml:Transition' && trTi.includes('unnamed'))
                LIB.updatePropertyByTitle(tr, DcProperty.title, () => { return [[{ text: sTi + '→' + tTi }]]; }, spD);
        });
