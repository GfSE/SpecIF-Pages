...
/*!	Transform SysML XMI to SpecIF
    ...
    ToDo:
    - !! extract the relation between uml:Actor and uml:UseCase
    - The events of state machines should be listed below the state machine in the hierarchy (one level lower); see xEvent.
    - an association named 'someName' now gets a statementClass "SpecIF:isAssociatedWith" and a type property with value 'someName'.
      Better create a statementClass with title 'someName' as specialization of "SpecIF:isAssociatedWith" and assign it to the statement.
    - Include Profile diagrams in the SpecIF data-set as well, to make the stereotypes and data types visible in the hierarchy.
*/
function sysml2specif(xmi, options) {
    ...
    [
        ...
        { id: "rcUseCase", type: "resourceClass", term: "uml:UseCase" },
        { id: "rcStereotype", type: "resourceClass", term: "uml:Stereotype" },
        { id: "scDefault", type: "statementClass", term: "SpecIF:isAssociatedWith" },
        ...
        { id: "scHasPart", type: "statementClass", term: `${CONFIG.pfxNsDcmi}hasPart` },
        ...
        { id: "scIncludes", type: "statementClass", term: "uml:Include" },
        { id: "scExtends", type: "statementClass", term: "uml:Extend" },
        { id: "scAbstracts", type: "statementClass", term: "uml:Abstraction" },
        { id: "scTraces", type: "statementClass", term: "uml:Trace" },
    ...
    if (validateCameo(xmiDoc)) {
        const spD = cameo2specif(xmiDoc, opts);
        return new resultMsg({ status: 0, statusText: 'Cameo Import successfully done', responseType: 'json', response: spD });
    }
    ;
    return new resultMsg({ status: 899, statusText: 'Cameo Import: Input file is not supported.' });
    function cameo2specif(xmi, opts) {
        ...
        }), diagramL = [], usedElementL = [], abstractions = [], specializations = [], associationEnds = [], portL = [], connectorL = [], stateTransitionL = [], pCType = LIB.itemById(spD.propertyClasses, 'PC-Type'), typeLength = LIB.itemByKey(spD.dataTypes, pCType.dataType).maxLength;
        for (let modDoc of modL) {
            ...
            let rId = modDoc.getAttribute("xmi:id"), r = {
                ...
                properties: [{
                        ...
                        values: [[{ text: (modDoc.getAttribute("xmi:type") ?? '').slice(-typeLength) }]]
                    }],
                changedAt: opts.fileDate
            }, nd = makeNode(r, spD.id);
            spD.resources.push(r);
            spD.nodes.push(nd);
            parseElements(modDoc, { packageId: rId, nodes: nd.nodes });
        }
        ;
        ...
        abstractions = abstractions
            ...
            if (sTy) {
                ...
                if (sC) {
                    ...
                    let prp = {
                        class: LIB.makeKey("PC-Type"),
                        values: [[{ text: sTy.tag.slice(-typeLength) }]]
                    };
        ...
        connectorL.forEach((co) => {
            const port0 = LIB.itemById(portL, co.ends[0]), port1 = LIB.itemById(portL, co.ends[1]);
            if (!port0 || !port1) {
                console.info(`Cameo import: skipping connector ${co.id}, because port at one or both ends cannot be found.`);
                return;
            }
            const p0serves = LIB.valueByTitle(port0.resource, 'uml:is_Service', spD) == 'true', p1serves = LIB.valueByTitle(port1.resource, 'uml:is_Service', spD) == 'true';
        ...
        stateTransitionL.forEach((stT) => {
            let tr = LIB.itemById(spD.resources, stT.id), trTi = LIB.valueByTitle(tr, CONFIG.propClassTitle, spD), trTy = LIB.valueByTitle(tr, CONFIG.propClassType, spD), src = LIB.itemById(spD.resources, stT.source), sTi = LIB.valueByTitle(src, CONFIG.propClassTitle, spD), sTy = LIB.valueByTitle(src, CONFIG.propClassType, spD), tgt = LIB.itemById(spD.resources, stT.target), tTi = LIB.valueByTitle(tgt, CONFIG.propClassTitle, spD);
            if (sTy == 'uml:Pseudostate' && sTi.includes('unnamed')) {
                sTi = 'entry';
                LIB.updatePropertyByTitle(src, CONFIG.propClassTitle, () => { return [[{ text: sTi }]]; }, spD);
            }
            ;
            if (trTy == 'uml:Transition' && trTi.includes('unnamed'))
                LIB.updatePropertyByTitle(tr, CONFIG.propClassTitle, () => { return [[{ text: sTi + '→' + tTi }]]; }, spD);
        });
        ...
        function parseElements(elem, params) {
            function addResource(r, prms, opts) {
                spD.resources.push(r);
                const nd = makeNode(r, prms.packageId);
                ...
                if (prms.packageId) {
                    spD.statements.push({
                        id: CONFIG.prefixS + 'contains-' + simpleHash(prms.packageId + terms.scContains + r.id),
                        class: LIB.makeKey(terms.scContains),
                        subject: LIB.makeKey(prms.packageId),
            ...
            Array.from(elem.children, (ch) => {
                switch (ch.tagName) {
                    ...
                    case "packagedElement":
                        ...
                        switch (ty) {
                            case "uml:DataType":
                                addResource(makeResource(ch, { class: terms.rcDefault }), params, opts);
                                break;
                            case "uml:Stereotype":
            ...
            Array.from(elem.children, (ch) => {
                ...
                switch (ch.tagName) {
                    ...
                    case "xmi:Extension":
                        ...
                        break;
                    case "packagedElement":
                        const ty = ch.getAttribute("xmi:type");
                        switch (ty) {
                            case "uml:DataType":
                            case "uml:Stereotype":
                            case "uml:Enumeration":
                            ...
                            case 'uml:Package':
                                ...
                                nd = addResource(r, params, opts);
                                parseElements(ch, { packageId: r.id, nodes: nd?.nodes });
                                break;
                            case 'uml:UseCase':
                            case 'uml:Actor':
                            case 'uml:Class':
                                xClass(ch, { ...params, type: ty });
                                break;
                            case 'uml:Activity':
                        ...
                        switch (ty) {
                            ...
                            case "uml:Abstraction":
                                ...
                                abstractions.push({
                                    ...
                                    class: LIB.makeKey(terms.scAbstracts),
            ...
            function xClass(el, params) {
                const cl = params.type == 'uml:UseCase' ? terms.rcUseCase : terms.rcDefault;
                let r2 = makeResource(el, { class: cl });
                const nd2 = addResource(r2, params, opts), pars = { packageId: params.packageId, parent: r2, nodes: nd2.nodes };
                Array.from(el.children, (ch) => {
                    switch (ch.tagName) {
                        ...
                        case 'nestedClassifier':
                            ...
                            break;
                        case 'ownedUseCase':
                            xClass(ch, { ...pars, type: 'uml:UseCase' });
                            break;
                        case 'ownedBehavior':
                        ...
                        case 'include':
                            const stIn = {
                                id: ch.getAttribute("xmi:id"),
                                class: LIB.makeKey(terms.scIncludes),
                                subject: LIB.makeKey(r2.id),
                                object: LIB.makeKey(ch.getAttribute("addition")),
                                changedAt: opts.fileDate
                            };
                            spD.statements.push(stIn);
                            break;
                        case 'extend':
                            const stEx = {
                                id: ch.getAttribute("xmi:id"),
                                class: LIB.makeKey(terms.scExtends),
                                subject: LIB.makeKey(r2.id),
                                object: LIB.makeKey(ch.getAttribute("extendedCase")),
                                changedAt: opts.fileDate
                            };
                            spD.statements.push(stEx);
                            break;
                        case 'extensionPoint':
                        default:
                            infoSkip(ch);
            ...
            function xEvent(el, pars) {
                const r = makeResource(el, { class: terms.rcEvent });
                addResource(r, pars, opts);
            }
            function xAssociation(el) {
                ...
                if (nm) {
                    ...
                    if (!sC)
                        prpL = [{
                                class: LIB.makeKey("PC-Type"),
                                values: [[{ text: replaceSeparatorNS(nm).slice(-typeLength) }]]
                            }];
                }
                ;
                if (aEnds.length == 1) {
                ...
                Array.from(el.getElementsByTagName('ownedDiagram'), (oD) => {
                    ...
                    const r = {
                        ...
                        properties: [{
                                ...
                                class: LIB.makeKey("PC-Type"),
                                values: [[{ text: (oD.getAttribute("xmi:type") ?? '').slice(-typeLength) }]]
                            }, {
            ...
            function makeStateMachine(el, params) {
                ...
                function makeRegion(rg, params) {
                    const reg = makeResource(rg, { class: terms.rcActor });
            ...
            function makeActivity(el, params) {
                const r = makeResource(el, { class: terms.rcActor }), nd = addResource(r, params, opts);
                if (params.parent)
                    spD.statements.push({
                        id: CONFIG.prefixS + 'ownedBehavior-' + simpleHash(params.parent.id + terms.scHasBehavior + r.id),
                        class: LIB.makeKey(terms.scHasBehavior),
                ...
                Array.from(el.children, (ch) => {
                    switch (ch.tagName) {
                        case "xmi:Extension":
                            makeDiagrams(ch, { ...params, parent: r });
                            break;
                        case "node":
                            ...
                            break;
                        case "ownedBehavior":
                            makeActivity(ch, { ...params, nodes: nd.nodes, parent: r });
                    }
                    ;
                });
                Array.from(el.children, (ch) => {
            ...
            function makeResourceClass(el, pars) {
                const rcId = el.getAttribute("xmi:id"), desc = parseDesc(el, { parent: rcId }), rc = {
            ...
            function makeResource(el, pars) {
                const r = {
                    ...
                    properties: [{
                            ...
                            class: LIB.makeKey("PC-Type"),
                            values: [[{ text: (el.getAttribute("xmi:type") ?? '').slice(-typeLength) }]]
                        }],
                    changedAt: opts.fileDate
                };
                addDesc(r, el);
