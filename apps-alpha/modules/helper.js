...
class resultMsg {
    constructor(xhr) {
        ...
        this.ok = xhr.ok || ["0", "200", "201"].includes(String(this.status));
    }
    ok() {
        return this.ok;
    }
...
LIB.stdError = (xhr, cb) => {
    let xhrCl = new resultMsg({ status: xhr.status, statusText: xhr.statusText, responseType: xhr.responseType, response: xhr.responseType == 'text' ? (xhr.response || xhr.responseText) : '' });
    switch (xhr.status) {
        case 0:
        case 200:
        case 201:
