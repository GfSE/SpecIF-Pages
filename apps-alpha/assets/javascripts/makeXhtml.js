...
//		if( Object.keys(sts).length<1 ) return '';
	...
	//		sTi;
        ...
        // First sort by relationship title (predicate), then process in a loop:
        Object.keys(sts).sort().forEach((cid) => {

			// 3 columns:
			if( sts[cid].subjects.length>0 ) {
                ct += '<tr><td>';
                // Sort and display subjects in left colums:
                LIB.sortBy(sts[cid].subjects, (s) => { return titleOf(s, undefined, opts) } );
