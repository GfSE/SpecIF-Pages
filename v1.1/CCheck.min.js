...
p(e,n))&&m.push(n)});q(m,k[["class"]])&&g.push({status:981,statusText:"the object of statement["+b+"] with identifier '"+c.id+"' has a class which is not listed in the objectClasses of the statement's class"})}}});y(f.statementClasses,f.statements,"class")})();z(f.resources,f.hierarchies,0);return 1>g.length?{status:0,statusText:"SpecIF constraints have been checked successfully!"}:{status:973,statusText:"SpecIF constraints are violated",responseType:"text",responseText:B(g)}}};