...
class CCheck {
    ...
    checkSchema( data, options ) {
        ...
        return valid?{ status: 0, statusText: 'SpecIF schema has been checked successfully!' }
                :{ status: 901, statusText: 'SpecIF schema is violated', responseType: 'text', responseText: this.ajv.errorsText(validate.errors) }
    }
    checkConstraints( data, options ) {
        "use strict";
        // Check the constraints of the concrete values in 'data'.
        // SpecIF schema v0.11.1 up until v1.0 is supported.
                ...
                iL.forEach( function( ins ){
                    if( ins.properties ) {
                        ...
                        ins.properties.forEach( function( prp ){
                            et = "property with class '"+(prp[pClass].id||prp[pClass])+"' of instance with identifier '"+ins.id+"'";
                            ...
                            if( data.propertyClasses ) {
                                ...
                                if( (!instanceC.propertyClasses || !itemByKey(instanceC.propertyClasses,prp['class']))
                                    && (!extendedC || !extendedC.propertyClasses || !itemByKey(extendedC.propertyClasses,prp['class']) ) )
                                ...
                                propertyC = itemByKey( instanceC[pClasses], prp[pClass] )
                            };
                            if( propertyC ) 
        ...
        function itemByKey( L, k ) {
            ...
            let _k = normalize(k);
			if( !_k || !Array.isArray(L) ) return // undefined
			let _L = L.map( normalize );
			
            // Find all elements with the same id:
            let itemsWithEqId = _L.filter( function(e) { return e.id==_k.id });
            ...
            if( itemsWithEqId.length==1 && !itemsWithEqId[0].revision ) {
                ...
                if( _k.revision ) return // revisions don't match (this case should not occur)
                else return itemsWithEqId[0] // both the found element and the key have no revision
            };
        
            // The elements in _L have a revision and there may be more than one of them.
			...
			function normalize(e) {
				switch( typeof(e) ) {
					...
					case 'string': return {id:e};
					default: return; // undefined in case of undefined or invalid key or id
				};
			}
        }  
