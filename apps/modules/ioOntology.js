...
class COntology {
    ...
    getTermResources(ctg, term, opts) {
        let nTerm = term.replace(/^dc(?:terms):/, CONFIG.pfxNsDcmi);
        ctg = ctg.toLowerCase();
    ...
    getPreferredTerm(ctg, term) {
        if (term.startsWith(CONFIG.pfxNsDcmi))
            return term;
        let tR = this.getTermResource(ctg, term);
        if (tR) {
    ...
    makeIdAndTitle(r, pfx) {
        ...
        let visIdL = LIB.valuesByTitle(r, [`${CONFIG.pfxNsDcmi}identifier`], this.data), prp = LIB.itemBy(r.properties, 'class', { id: termId });
